function createDataset(fields, constraints, sortFields) {
    ...
    try {
        ...
        var fileName = "EMPRESA_FILIAL.CSV"; // Arquivo que queremos achar
        ...
        if (dsDocs != null && dsDocs.rowsCount > 0) {
            for (var i = 0; i < dsDocs.rowsCount; i++) {
                ...
                // Se o nome ou a descrição bater com o nosso CSV
        ...
        // --- TRUQUE DEFINITIVO: Conversão para Tipos Primitivos do Java ---
        // Forçamos a criação de Inteiros nativos do Java (.intValue()) para não haver chance
        // do JaxWsClientProxy recusar a assinatura do método.
        ...
        // --- 4. Baixar o arquivo (Pulamos o getDocument pois a busca do passo 2 já nos deu os dados) ---
        ...
        // --- 5. Converter o conteúdo para Texto ---
        // Criamos a String no Java (ISO-8859-1 costuma preservar melhor os acentos do TOTVS RM)
        var textoJava = new java.lang.String(byteContent, "ISO-8859-1"); 
        
        // O pulo do gato: forçamos a conversão para String nativa do JavaScript
        var textoJS = String(textoJava);
        
        // --- 6. Processar o CSV ---
        // Agora o split do Javascript vai encontrar todas as quebras de linha corretamente
        ...
        for (var i = 0; i < linhas.length; i++) {
            ...
            if (isFirstLine) {
                // Cria as colunas baseadas na primeira linha
                colCount = columns.length;
                for (var c = 0; c < colCount; c++) {
                    dataset.addColumn(String(columns[c]).replace(/^"|"$/g, '').trim());
                }
                isFirstLine = false;
            } else {
                // Preenche as linhas de dados (Empresas e Filiais)
